...
type postgresPrimeRepository struct {
	db    *sql.DB
	prime *prime.Filter
}

func NewPostgresPrimeRepository(db *sql.DB, prime *prime.Filter) PrimeRepository {
	return &postgresPrimeRepository{db: db, prime: prime}
}

func (m *postgresPrimeRepository) GetAllCustomers(request resource.CustomersListRequest) (*resource.CustomersListResponse, error) {
	...
	if request.First != "" && request.Rows != "" {
		dQuery = dQuery + fmt.Sprintf(limitSql, fmt.Sprintf("$%d", len(vals)+1), fmt.Sprintf("$%d", len(vals)+2))
...
func (m *postgresPrimeRepository) GetAllRepresentatives() ([]*model.Representative, error) {
